...
>
 ...
 return (
 <div className="relative flex min-h-screen w-full flex-col bg-[#f9f9fd] #0a0f0d] text-slate-900 transition-colors duration-300">
 ...
 <p className="text-slate-600 font-medium max-w-xl text-lg">
 Comprehensive performance breakdown for your recent assessment.
 </p>
 </div>

 {/* Bento-style Score Card */}
 <div className="bg-white p-6 md:p-8 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-6 sm:gap-8 min-w-fit md:min-w-[320px]">
...
></circle>
 ...
 <span className="absolute text-xl md:text-2xl font-extrabold text-slate-900 ">{percentage}%</span>
 </div>
 ...
 <span className="text-xs font-bold text-slate-600 ">Time: {timeSpent}</span>
 </div>
 </div>
 </div>
 </header>
 ...
 <h2 className="text-2xl font-extrabold tracking-tight text-slate-900 ">Cognitive Taxonomy Breakdown</h2>
 ...
 return (
 <div key={level} className={`bg-white border border-slate-100 p-5 md:p-6 rounded-2xl transition-all hover:-translate-y-1 shadow-sm ${!hasQuestions ? 'opacity-50' : ''}`}>
 <div className="text-[10px] font-bold text-slate-500 tracking-widest uppercase mb-3">{level}</div>
 {hasQuestions ? (
 <>
 <div className="text-2xl md:text-3xl font-extrabold text-slate-900 mb-4">{levelPercentage}%</div>
 ...
 <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 border-b-2 border-slate-100 pb-4">
 <h2 className="text-2xl font-extrabold tracking-tight text-slate-900 mb-4 sm:mb-0">Question Breakdown</h2>
 <div className="flex gap-4 sm:gap-6 bg-slate-50 p-1.5 rounded-xl border border-slate-200">
            <button 
 ...
 return (
 <div key={index} className={`group bg-white border ${isCorrect ? 'border-slate-100 border-l-4 border-l-[#95ff00]' : 'border-red-500/20 border-l-4 border-l-red-500'} p-6 md:p-8 rounded-3xl shadow-sm hover:shadow-md transition-all`}>
 <div className="flex justify-between items-start mb-6">
 <span className="bg-slate-100 text-slate-700 px-3 py-1 rounded-full text-[10px] font-extrabold tracking-widest uppercase">
 {question.level || "General"}
 </span>
 <div className={`flex items-center gap-2 font-bold text-sm ${isCorrect ? 'text-green-600 #95ff00]' : 'text-red-500'}`}>
 ...
 {/* User Answer block */}
 {wasAnswered && (
 <div className={`flex items-center gap-4 p-4 md:p-5 rounded-2xl ${
 isCorrect
 ...
 <span className={`font-semibold text-sm md:text-base ${isCorrect ? 'text-slate-900 ' : 'text-red-800 '}`}>
 ...
 {/* Footer Actions */}
 <section className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4 pb-12 border-t border-slate-100 ">
 <motion.button
 onClick={() => navigate("/dashboard")}
 className="w-full sm:w-auto px-8 py-4 bg-white text-slate-900 font-bold rounded-full border border-slate-200 hover:bg-slate-100 :bg-white/10 transition-colors shadow-sm"
...
>
 ...
 className="w-full sm:w-auto px-8 py-4 bg-slate-900 #95ff00] text-white font-extrabold rounded-full shadow-xl shadow-black/10 #95ff00]/20 hover:scale-105 transition-transform"
 whileHover={{ scale: 1.02}}
 whileTap={{ scale: 0.98}}
>
 Retake Quiz
 ...
 <h1 className="text-slate-900 font-extrabold text-2xl md:text-3xl lg:text-4xl leading-[1.3] tracking-tight text-balance p-4 whitespace-normal break-words max-w-full">
 ...
 className={`group relative w-full text-left p-6 md:p-8 transition-all duration-300 flex items-center gap-4 md:gap-8 rounded-2xl ${
 isSelected 
 ? "bg-[#95ff00] ring-4 ring-[#95ff00]/50 ring-offset-4 ring-offset-[#f9f9fd] #0a0f0d] shadow-xl shadow-[#95ff00]/20" 
 : "bg-white hover:bg-slate-50 :bg-white/5 border border-slate-100 "
}`}
>
 <div className={`flex-shrink-0 w-12 h-12 rounded-xl flex items-center justify-center font-bold text-xl transition-colors ${
 isSelected
 ? "bg-black/20 text-black"
 : "bg-slate-100 text-slate-700 group-hover:bg-white :bg-white/20"
}`}>
 {letters[index]}
 </div>
